...
| AI | **NVIDIA NIM** — `https://integrate.api.nvidia.com/v1` (OpenAI-compatible) | Free 1,000 dev credits (top up at launch). Chat: **`nvidia/nemotron-3-super`** (Nemotron-3-Super-120B-A12B — 120B total/12B active MoE, 1M ctx; Karan's pick). NVIDIA guidance: `temperature=1.0, top_p=0.95` for all tasks; reasoning toggle `enable_thinking` via `chat_template_kwargs` (benchmark off vs `low_effort=True` for chat latency in Phase A). **Phase A smoke test:** verify it accepts `extra_body={"nvext":{"guided_json": schema}}` — if not, fallback `meta/llama-3.3-70b-instruct` (guided_json confirmed) for the itinerary-emitting call only (Nemotron chats, Llama emits the card). Embeddings: `nvidia/nv-embedqa-e5-v5` (1024-dim; **512-token input cap**; asymmetric — `input_type:"passage"` at ingestion, `"query"` at retrieval) |
